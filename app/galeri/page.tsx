            ...
            <section className="w-full pb-32 px-6 flex-1">
                <div className="max-w-6xl mx-auto pt-16 relative min-h-[500px] flex items-center justify-center">
                    ...
                    <div className="absolute inset-0 grid grid-cols-2 md:grid-cols-3 gap-4 pt-16 opacity-10 pointer-events-none select-none">
                        {[1, 2, 3, 4, 5, 6].map((i) => (
                            <div key={i} className="aspect-square bg-[#111113] border border-white/10 rounded-sm"></div>
                        ))}
                    </div>

                    <ScrollReveal delay={100} className="relative z-10 flex flex-col items-center text-center p-6 bg-[#09090b]/60 backdrop-blur-sm rounded-sm border border-white/5 shadow-2xl">
