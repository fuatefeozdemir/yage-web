        ...
        <main className="flex flex-col min-h-screen bg-brand-bg">
            <section className="w-full pt-40 pb-16 px-6 border-b border-white/5">
                <div className="max-w-6xl mx-auto">
                    <ScrollReveal>
                        <h1 className="text-5xl md:text-7xl font-bold text-white tracking-tight mb-6">İletişim</h1>
                                ...
                                {status === "success" ? (
                                    ...
                                    <form className="flex flex-col gap-5" onSubmit={handleSubmit}>
                                        <div className="flex flex-col gap-1.5">
                                            ...
                                            <input type="text" id="name" required disabled={status === "loading"} value={formData.name} onChange={handleChange} className="w-full bg-brand-surface border border-white/10 rounded-sm p-3.5 text-sm text-white placeholder:text-brand-muted/30 focus:outline-none focus:border-brand-primary transition-colors disabled:opacity-50" placeholder="Adınız Soyadınız" />
                                        </div>
                                        <div className="flex flex-col gap-1.5">
                                            <label htmlFor="email" className="text-xs font-medium text-brand-muted">E-posta</label>
                                            <input type="email" id="email" required disabled={status === "loading"} value={formData.email} onChange={handleChange} className="w-full bg-brand-surface border border-white/10 rounded-sm p-3.5 text-sm text-white placeholder:text-brand-muted/30 focus:outline-none focus:border-brand-primary transition-colors disabled:opacity-50" placeholder="ornek@mail.com" />
                                        </div>
                                        <div className="flex flex-col gap-1.5 relative">
                                            <label htmlFor="subject" className="text-xs font-medium text-brand-muted">Konu</label>
                                            <select id="subject" required disabled={status === "loading"} value={formData.subject} onChange={handleChange} className="w-full bg-brand-surface border border-white/10 rounded-sm p-3.5 text-sm text-white focus:outline-none focus:border-brand-primary transition-colors appearance-none cursor-pointer disabled:opacity-50">
                                        ...
                                        <div className="flex flex-col gap-1.5">
                                            ...
                                            <textarea id="message" required rows={5} disabled={status === "loading"} value={formData.message} onChange={handleChange} className="w-full bg-brand-surface border border-white/10 rounded-sm p-3.5 text-sm text-white placeholder:text-brand-muted/30 focus:outline-none focus:border-brand-primary transition-colors resize-none disabled:opacity-50" placeholder="Mesajınızı buraya yazın..."></textarea>
                                        </div>
                                        {status === "error" && (
                                            <div className="flex items-center gap-2 text-red-400 text-sm bg-red-400/10 p-3 rounded-sm border border-red-400/20">
                                                <XCircle size={16} /><span>Mesaj gönderilemedi. Lütfen tekrar deneyin.</span>
                                            </div>
                                        )}
                                        <div className="mt-2">
                                            <button type="submit" disabled={status === "loading"} className="w-full md:w-auto flex justify-center items-center gap-2 bg-brand-primary text-white px-10 py-3.5 text-sm font-bold hover:bg-white hover:text-black transition-colors duration-300 rounded-sm disabled:opacity-70 disabled:hover:bg-brand-primary disabled:hover:text-white">
            ...
            <section className="w-full border-t border-white/5 py-24 px-6 bg-brand-surface-dark">
                <ScrollReveal>
                    <div className="max-w-6xl mx-auto">
                        <div className="mb-10 text-center">
                            <h2 className="text-2xl font-bold text-white tracking-tight mb-2">Bizi Bul</h2>
                            <p className="text-brand-muted text-base">{siteConfig.iletisim.adresDetay}</p>
                        </div>
                        <div className="w-full h-[320px] bg-brand-surface border border-white/5 rounded-sm relative overflow-hidden">
                            ...
                            <iframe src="https://www.google.com/maps/embed?pb=!1m18!1m12!1m3!1d3058.847525287413!2d32.8188168!3d39.9431804!2m3!1f0!2f0!3f0!3m2!1i1024!2i768!4f13.1!3m3!1m2!1s0x14d34ece75a18115%3A0x6732313d8d64117!2sGazi%20%C3%9Cniversitesi%20Teknoloji%20Fak%C3%BCltesi!5e0!3m2!1str!2str!4v1700000000000!5m2!1str!2str" width="100%" height="100%" style={{ border: 0 }} allowFullScreen={false} loading="lazy" referrerPolicy="no-referrer-when-downgrade" className="relative z-10 grayscale opacity-70"></iframe>
                        </div>
                    </div>
                </ScrollReveal>
            </section>
