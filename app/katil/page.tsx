        ...
        <main className="flex flex-col min-h-screen bg-brand-bg">
            ...
            <section className="w-full pb-32 px-6 pt-12">
                <div className="max-w-3xl mx-auto">
                    <div className="bg-brand-surface border border-white/5 rounded-sm p-6 md:p-12 shadow-lg shadow-black/20">
                        {status === "success" ? (
                            <ScrollReveal>
                            ...
                            <ScrollReveal delay={50}>
                                <form className="flex flex-col gap-10" onSubmit={handleSubmit}>
                                    <div className="text-center md:text-left mb-2">
                                        <p className="text-sm text-brand-muted/80 leading-relaxed">YAGE'de yer almak istediğin alanı seçerek başvurunu tamamlayabilirsin.</p>
                                    </div>
                                    ...
                                    <div className="flex flex-col gap-5">
                                        ...
                                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                            <div className="flex flex-col gap-1.5 md:col-span-2">
                                                ...
                                                <input type="text" id="name" required value={formData.name} onChange={handleChange} disabled={status === "loading"} className="w-full bg-brand-surface-light border border-white/5 p-3.5 text-sm text-white focus:outline-none focus:border-brand-primary/50 transition-colors rounded-sm placeholder:text-brand-muted/30" placeholder="Adınız Soyadınız" />
                                            </div>
                                            ...
                                            <div className="flex flex-col gap-1.5 relative">
                                                ...
                                                <select id="grade" required value={formData.grade} onChange={handleChange} disabled={status === "loading"} className="w-full bg-brand-surface-light border border-white/5 p-3.5 text-sm text-white focus:outline-none focus:border-brand-primary/50 transition-colors appearance-none cursor-pointer rounded-sm">
                                                    ...
                                                    <button
                                                        ...
                                                        className={`p-4 text-left border rounded-sm transition-all duration-200 ${
                                                            formData.primaryUnit === unit
                                                                ...
                                                                : "border-white/5 bg-brand-surface-light text-brand-muted hover:border-white/20 hover:bg-brand-surface-lighter"
                                                        }`}
                                                    >
                                        ...
                                        {formData.primaryUnit && (
                                            <div className="mt-2 pt-6 border-t border-white/5 animate-fade-in-up flex flex-col gap-6">
                                                <div>
                                                    ...
                                                    <p className="text-sm text-brand-muted/80 leading-relaxed">{unitDetails[formData.primaryUnit].desc}</p>
                                                </div>
                                                <div className="flex flex-col gap-3">
                                                    <h4 className="text-sm font-bold text-white">Bu birimle ilgili</h4>
                                                    <p className="text-sm text-brand-muted/90 leading-relaxed">{unitDetails[formData.primaryUnit].question} <span className="text-red-500">*</span></p>
                                                    <textarea id="unitAnswer" required rows={3} value={formData.unitAnswer} onChange={handleChange} disabled={status === "loading"} className="w-full bg-brand-surface-light border border-white/5 p-4 text-sm text-white focus:outline-none focus:border-brand-primary/50 transition-colors rounded-sm mt-1 resize-none placeholder:text-brand-muted/30" placeholder="Kısaca anlatabilirsin..."></textarea>
                                                </div>
                                            </div>
                                        )}
                                                        ...
                                                        <button
                                                            ...
                                                            className={`px-3 py-2 text-[11px] font-medium border rounded-sm transition-colors ${
                                                                formData.secondaryUnits.includes(unit)
                                                                    ...
                                                                    : "border-white/5 text-brand-muted bg-brand-surface-light hover:bg-brand-surface-lighter"
                                                            }`}
                                                        >
                                                            {unit}
                                                        </button>
                                    ...
                                    <div className="flex flex-col gap-5">
                                        ...
                                        <div className="flex flex-col gap-1.5">
                                            ...
                                            <textarea id="experiences" rows={3} value={formData.experiences} onChange={handleChange} disabled={status === "loading"} className="w-full bg-brand-surface-light border border-white/5 p-4 text-sm text-white focus:outline-none focus:border-brand-primary/50 transition-colors rounded-sm resize-none placeholder:text-brand-muted/30" placeholder="Daha önce yer aldığın topluluklar, geliştirdiğin projeler veya yaptığın çalışmalar varsa bahsedebilirsin."></textarea>
                                        </div>
                                        ...
                                        <div className="flex flex-col gap-1.5 mt-1">
                                            ...
                                            <textarea id="additionalInfo" rows={2} value={formData.additionalInfo} onChange={handleChange} disabled={status === "loading"} className="w-full bg-brand-surface-light border border-white/5 p-4 text-sm text-white focus:outline-none focus:border-brand-primary/50 transition-colors rounded-sm resize-none placeholder:text-brand-muted/30" placeholder="Eklemek istediğin başka bir bilgi varsa paylaşabilirsin."></textarea>
                                        </div>
                                    </div>
                                    ...
                                    {status === "error" && (
                                        <div className="flex items-center gap-2 text-red-400 text-sm bg-red-400/10 p-3 rounded-sm border border-red-400/20">
                                            <XCircle size={16} />
                                            <span>Başvuru gönderilemedi veya hız sınırına takıldınız. Lütfen tekrar deneyin.</span>
                                        </div>
                                    )}
